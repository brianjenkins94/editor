...
import type { FsShim } from "./shims/fs";
...
import { createProcess } from "./shims/process";
...
function transformEsmToCjsAst(code: string, filename: string): string {
  ...
  // Collect deep replacements: import.meta → import_meta, import() → __dynamicImport()
	const deepReplacements: [number, number, string][] = [];
  ...
  // Check for actual import/export declarations
	...
	const hasExportDecl = sourceFile.statements.some((statement) => ts.isExportDeclaration(statement)
...
function transformEsmToCjsRegexFallback(code: string, filename: string): string {
  ...
  // Transform import/export (AST with its own regex fallback)
	const hasImport = /\bimport\s+[\w{*'"]/.test(code);
 ...
 */
// A NATIVE dynamic import, built with `new Function` so neither the bundler nor almostnode's own
...
const builtinModules: Record<string, unknown> = {
  ...
  // Some packages explicitly require 'console' (with Console constructor)
	"console": {
		...
		"Console": class Console {
			private readonly _stdout: { "write": (s: string) => void } | null;
			private readonly _stderr: { "write": (s: string) => void } | null;
        ...
        // Node's Console accepts (stdout, stderr) or { stdout, stderr }
          ...
          // new Console({ stdout, stderr })
			...
			private _write(stream: "out" | "err", args: unknown[]) {
				...
				if (target) { target.write(msg); } else if (stream === "err") { console.error(...args); } else { console.log(...args); }
			}
  ...
  // timers subpaths
	"timers/promises": {
		...
		"setInterval": globalThis.setInterval,
		"setImmediate": (value?: unknown) => new Promise((resolve) => { setTimeout(resolve, 0, value); }),
      ...
      // Also check without extension for common patterns
			...
			for (const key of [relPath, relPathNoExt]) {
				if (key in browserMap) {
					...
					return tryResolveFile(pathShim.join(pkgRoot, browserMap[key]));
				}
			}
...
${code}
      ...
      // Create dynamic import function for this module context
			...
			fn(
				...
				process,
				consoleWrapper,
				{ "url": importMetaUrl, "dirname": dirname, "filename": resolvedPath },
				dynamicImport
			);
...
export class Runtime {
    ...
    // Browser timers return plain numbers. Many npm packages (vitest, etc.) call .unref()
		if (!(globalThis.setTimeout as any).__patched) {
			...
			const wrapTimer = (id: ReturnType<typeof origSetTimeout>) => {
				const t = { "_id": id, "ref": function() { return t; }, "unref": function() { return t; }, "hasRef": function() { return true; }, "refresh": function() { return t; }, [Symbol.toPrimitive]: function() { return id; } };
   ...
   */
	private setupTextDecoderPolyfill(): void {
		...
		class PolyfillTextDecoder {
			...
			private readonly decoder: TextDecoder | null = null;
   ...
   */
    ...
    // Parse a stack trace string into structured frames
		function parseStack(stack: string): { "fn": string; "file": string; "line": number; "col": number }[] {
			if (!stack) { return []; }
			const frames: { "fn": string; "file": string; "line": number; "col": number }[] = [];
        ...
        // eslint-disable-next-line no-useless-assignment -- defensive defaults; each format branch below fills all four before pushing a frame.
				let colNo = 0; let file = ""; let fn = ""; let lineNo = 0;

        // Safari format: "functionName@file:line:col" or "@file:line:col"
				const safariMatch = /^(.*)@(.*?):(\d+):(\d+)$/.exec(line);
				...
				if (safariMatch) {
					...
					frames.push({ "fn": fn, "file": file, "line": lineNo, "col": colNo });
					continue;
				}

        // Chrome format: "at functionName (file:line:col)" or "at file:line:col"
				const chromeMatch = /^at\s+(?:(.+?)\s+\()?(.*?):(\d+):(\d+)\)?$/.exec(line);
...
${code}
      ...
      // eslint-disable-next-line no-eval -- executing module code is almostnode's whole purpose; the runtime IS an evaluator.
			...
			fn(
				...
				this.process,
				consoleWrapper,
				{ "url": importMetaUrl, "dirname": dirname, "filename": filename },
				dynamicImport
			);
		...
		return { "exports": module.exports, "module": module };
	}
...
while (true) {
  ...
  try {
    ...
    yield { value: undefined, error: e };
		...
		return {
			"eval": function(code: string): unknown {
...
// Re-export types
export type { IExecuteResult, IRuntime, IRuntimeOptions } from "./runtime-interface";
