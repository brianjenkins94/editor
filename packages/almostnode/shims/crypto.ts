	...
	verify(publicKey: KeyLike, signature: Buffer | string, signatureEncoding?: string): boolean {
		...
		if (typeof signature === "string") {
			if (signatureEncoding === "base64") {
				...
				sig = Buffer.from(signature.match(/.{2}/g).map((byte) => Number.parseInt(byte, 16)));
			} else {
...
export class KeyObject {
	private readonly _keyData: CryptoKey | Uint8Array;
	private readonly _type: "public" | "private" | "secret";
...
function getWebCryptoAlgorithm(nodeAlgorithm: string): { "name": string; "hash"?: string } {
  ...
  // RSA algorithms
	if (alg.includes("RSA")) {
		if (alg.includes("PSS")) {
			const hash = (/SHA(\d+)/.exec(alg))?.[0] || "SHA-256";
  ...
  // ECDSA algorithms (ES256, ES384, ES512)
	if (alg.startsWith("ES") || alg.includes("ECDSA")) {
		const bits = (/\d+/.exec(alg))?.[0] || "256";
		const hash = bits === "512" ? "SHA-512" : bits === "384" ? "SHA-384" : "SHA-256";

		return { "name": "ECDSA", "hash": hash };
	}
...
function extractKeyInfo(key: KeyLike): KeyInfo {
  ...
  // Detect PEM format
    ...
    // Try to detect algorithm from key header
		...
		if (keyStr.includes("RSA")) { algorithm = "RSA-SHA256"; } else if (keyStr.includes("EC")) { algorithm = "ES256"; } else if (keyStr.includes("ED25519")) { algorithm = "Ed25519"; }
