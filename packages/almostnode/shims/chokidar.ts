...
import type { FSWatcher as VFSWatcher, VirtualFS } from "../virtual-fs";
...
export interface ChokidarOptions {
	"persistent"?: boolean;
	"ignored"?: string | RegExp | ((path: string) => boolean) | (string | RegExp | ((path: string) => boolean))[];
...
export class FSWatcher extends EventEmitter {
	...
	private readonly watched = new Map<string, VFSWatcher>();
	private readonly options: ChokidarOptions;
	...
	private collectDirContents(dirPath: string, pendingEmits: (() => void)[]): void {
		try {
...
export default { "watch": watch, "FSWatcher": FSWatcher, "setVFS": setVFS };
