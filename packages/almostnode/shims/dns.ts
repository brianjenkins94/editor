...
type LookupAllCallback = (err: Error | null, addresses?: { "address": string; "family": number }[]) => void;
...
export const promises = {
	"lookup": (hostname: string, options?: { "family"?: number; "all"?: boolean }) => {
		return new Promise((resolve, reject) => {
			if (options?.all) {
				lookup(hostname, options, ((err: Error | null, addresses?: { "address": string; "family": number }[]) => {
			...
			lookup(hostname, options || {}, (err, address, family) => {
				if (err) { reject(err); } else { resolve({ "address": address, "family": family }); }
			});
		});
	},
	"resolve": (hostname: string) => {
		return new Promise<string[]>((promiseResolve, promiseReject) => {
			resolve(hostname, (err, addresses) => {
				if (err) { promiseReject(err); } else { promiseResolve(addresses || []); }
			});
		});
	},
