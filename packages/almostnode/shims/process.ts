...
function createProcessStream(
	...
	const stream: ProcessWritableStream & ProcessReadableStream = {
		...
		"on": function(event: string, listener: EventListener) {
			emitter.on(event, listener);

			return stream;
		},
		"once": function(event: string, listener: EventListener) {
			emitter.once(event, listener);

			return stream;
		},
		"off": function(event: string, listener: EventListener) {
		...
		"addListener": function(event: string, listener: EventListener) {
			emitter.addListener(event, listener);

			return stream;
		},
		"removeListener": function(event: string, listener: EventListener) {
		...
		"setMaxListeners": function(n: number) {
		...
		"listenerCount": function(event: string) {
			return emitter.listenerCount(event);
		},
		"listeners": function(event: string) {
			return emitter.listeners(event);
		},
		"rawListeners": function(event: string) {
			return emitter.rawListeners(event);
		},
		"prependListener": function(event: string, listener: EventListener) {
			emitter.prependListener(event, listener);

			return stream;
		},
		"prependOnceListener": function(event: string, listener: EventListener) {
		...
		"setEncoding": function(_encoding: string) {
			return stream;
		},
    // Default write implementation (no-op for readable streams)
		"write": function(_data: string | Buffer, _encoding?: string, callback?: () => void) {
			if (callback) { queueMicrotask(callback); }

			return true;
		},
		"end": function(_data?: string, callback?: () => void) {
			if (callback) { queueMicrotask(callback); }
		},
  ...
  // Create an EventEmitter for process events
	...
	const proc: Process = {
      ...
      // Debug: Log cwd calls (limited to first 5)
		...
		"chdir": function(directory: string) {
    ...
    // EventEmitter methods - delegate to emitter but return proc for chaining
		"on": function(event: string, listener: EventListener): Process {
			emitter.on(event, listener);

			return proc;
		},

		"once": function(event: string, listener: EventListener): Process {
			emitter.once(event, listener);

			return proc;
		},

		"off": function(event: string, listener: EventListener): Process {
			emitter.off(event, listener);

			return proc;
		},

		"emit": function(event: string, ...args: unknown[]): boolean {
			return emitter.emit(event, ...args);
		},

		"addListener": function(event: string, listener: EventListener): Process {
			emitter.addListener(event, listener);

			return proc;
		},

		"removeListener": function(event: string, listener: EventListener): Process {
			emitter.removeListener(event, listener);

			return proc;
		},

		"removeAllListeners": function(event?: string): Process {
			emitter.removeAllListeners(event);

			return proc;
		},

		"listeners": function(event: string): EventListener[] {
			return emitter.listeners(event);
		},

		"listenerCount": function(event: string): number {
			return emitter.listenerCount(event);
		},

		"prependListener": function(event: string, listener: EventListener): Process {
			emitter.prependListener(event, listener);

			return proc;
		},

		"prependOnceListener": function(event: string, listener: EventListener): Process {
			emitter.prependOnceListener(event, listener);
		...
		"setMaxListeners": function(n: number): Process {
			emitter.setMaxListeners(n);
