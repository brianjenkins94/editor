...
interface BrotliModule { "compress": (data: Uint8Array) => Uint8Array; "decompress": (data: Uint8Array) => Uint8Array }
...
async function loadBrotli(): Promise<BrotliModule | null> {
	...
	if (!brotliLoadPromise) {
        ...
        // @vite-ignore — brotli-wasm is not a vendored dependency. The editor's language servers only use
        // the gzip/deflate paths (pako); brotli is lazy and unreached, so keep it out of the bundle/deps.
				const brotliWasmModule = await import(/* @vite-ignore */ "brotli-wasm");
