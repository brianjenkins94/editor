...
export type { FSWatcher, Stats, WatchEventType, WatchListener };
...
export interface FsShim {
	"readFileSync": ((path: PathLike) => Buffer) & ((path: PathLike, encoding: "utf8" | "utf-8") => string) & ((path: PathLike, options: { "encoding": "utf8" | "utf-8" }) => string) & ((path: PathLike, options: { "encoding"?: null }) => Buffer);
	"writeFileSync": (path: PathLike, data: string | Uint8Array) => void;
	"existsSync": (path: PathLike) => boolean;
	"mkdirSync": (path: PathLike, options?: { "recursive"?: boolean }) => void;
	"readdirSync": ((path: PathLike) => string[]) & ((path: PathLike, options: { "withFileTypes": true }) => Dirent[]) & ((path: PathLike, options?: { "withFileTypes"?: boolean; "encoding"?: string } | string) => string[] | Dirent[]);
	...
	"watch": ((filename: string, options?: { "persistent"?: boolean; "recursive"?: boolean }, listener?: WatchListener) => FSWatcher) & ((filename: string, listener?: WatchListener) => FSWatcher);
	"readFile": ((path: string, callback: (err: Error | null, data?: Uint8Array) => void) => void) & ((path: string, options: { "encoding": string }, callback: (err: Error | null, data?: string) => void) => void);
	...
	"access": ((path: string, callback: (err: Error | null) => void) => void) & ((path: string, mode: number, callback: (err: Error | null) => void) => void);
...
export interface FsPromises {
	"readFile": ((path: PathLike) => Promise<Buffer>) & ((path: PathLike, encoding: "utf8" | "utf-8") => Promise<string>) & ((path: PathLike, options: { "encoding": "utf8" | "utf-8" }) => Promise<string>);
...
export function createFsShim(vfs: VirtualFS, getCwd?: () => string, beforeFsWrite?: (op: "write", method: string, path: string) => void): FsShim {
  // Helper to resolve paths with cwd
	...
	const promises: FsPromises = {
		"readFile": function(pathLike: unknown, encodingOrOptions?: string | { "encoding"?: string | null }): Promise<Buffer | string> {
		...
		"writeFile": function(pathLike: unknown, data: string | Uint8Array): Promise<void> {
			return new Promise((resolve, reject) => {
				try {
		...
		"stat": function(pathLike: unknown): Promise<Stats> {
			return new Promise((resolve, reject) => {
				try {
		...
		"lstat": function(pathLike: unknown): Promise<Stats> {
			return this.stat(resolvePath(pathLike));
		},
		"readdir": function(pathLike: unknown, options?: { "withFileTypes"?: boolean } | string): Promise<string[] | Dirent[]> {
			return new Promise((resolve, reject) => {
				try {
					const path = resolvePath(pathLike);
		...
		"mkdir": function(pathLike: unknown, options?: { "recursive"?: boolean }): Promise<void> {
			return new Promise((resolve, reject) => {
				try {
		...
		"unlink": function(pathLike: unknown): Promise<void> {
			return new Promise((resolve, reject) => {
				try {
		...
		"access": function(path: string, mode?: number): Promise<void> {
			return new Promise((resolve, reject) => {
				try {
		...
		"copyFile": function(src: string, dest: string): Promise<void> {
			return new Promise((resolve, reject) => {
				try {
	...
	const shim = {
		...
		"writeFileSync": function(pathLike: unknown, data: string | Uint8Array): void {
      // Handle file descriptor
		...
		"existsSync": function(pathLike: unknown): boolean {
			return vfs.existsSync(resolvePath(pathLike));
		},

		"mkdirSync": function(pathLike: unknown, options?: { "recursive"?: boolean }): void {
			const path = resolvePath(pathLike);

			vfs.mkdirSync(path, options);
		},

		"readdirSync": function(pathLike: unknown, options?: { "withFileTypes"?: boolean; "encoding"?: string } | string): string[] | Dirent[] {
			const path = resolvePath(pathLike);
      ...
      // Debug: Log readdirSync results for _generated
		...
		"statSync": function(pathLike: unknown): Stats {
      ...
      // Debug: Log all statSync calls on _generated paths (show if path was modified)
		...
		"lstatSync": function(pathLike: unknown): Stats {
			return vfs.lstatSync(resolvePath(pathLike));
		},

		"fstatSync": function(fd: number): Stats {
			const entry = fdMap.get(fd);
		...
		"openSync": function(pathLike: unknown, flags: string | number, _mode?: number): number {
			const path = resolvePath(pathLike);
      ...
      // Write back content if it was opened for writing
		...
		"readSync": function(fd: number, buffer: Buffer | Uint8Array, offset: number, length: number, position: number | null): number {
			const entry = fdMap.get(fd);
		...
		"writeSync": function(fd: number, buffer: Buffer | Uint8Array | string, offset?: number, length?: number, position?: number | null): number {
			const entry = fdMap.get(fd);
      ...
      // Handle string input
			...
			const writePos = position ?? entry.position;
      ...
      // Write data
		...
		"ftruncateSync": function(fd: number, len = 0): void {
			const entry = fdMap.get(fd);
		...
		"fsyncSync": function(_fd: number): void {
      // No-op - our virtual FS doesn't have disk buffering
		},

		"fdatasyncSync": function(_fd: number): void {
      // No-op - our virtual FS doesn't have disk buffering
		},

		"mkdtempSync": function(prefix: string): string {
      // Generate a unique suffix
		...
		"rmSync": function(pathLike: unknown, options?: { "recursive"?: boolean; "force"?: boolean }): void {
			const path = resolvePath(pathLike);
		...
		"unlinkSync": function(pathLike: unknown): void {
			const path = resolvePath(pathLike);
      ...
      // Debug: Log unlink calls on _generated
		...
		"rmdirSync": function(pathLike: unknown): void {
			vfs.rmdirSync(resolvePath(pathLike));
		},

		"renameSync": function(oldPathLike: unknown, newPathLike: unknown): void {
		...
		"realpathSync": Object.assign(
			...
			{
				"native": function(pathLike: unknown): string {
					return vfs.realpathSync(resolvePath(pathLike));
				}
			}
		),

		"accessSync": function(pathLike: unknown, _mode?: number): void {
			vfs.accessSync(resolvePath(pathLike));
		},

		"copyFileSync": function(srcLike: unknown, destLike: unknown): void {
		...
		"stat": function(pathLike: unknown, callback: (err: Error | null, stats?: Stats) => void): void {
			vfs.stat(resolvePath(pathLike), callback);
		},

		"lstat": function(pathLike: unknown, callback: (err: Error | null, stats?: Stats) => void): void {
		...
		"realpath": function(pathLike: unknown, callback: (err: Error | null, resolvedPath?: string) => void): void {
		...
		"createReadStream": function(pathLike: unknown): unknown {
			return vfs.createReadStream(resolvePath(pathLike));
		},

		"createWriteStream": function(pathLike: unknown): unknown {
