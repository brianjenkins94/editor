...
import type { ResponseData,	Server } from "./shims/http";
...
export class ServerBridge extends EventEmitter {
   ...
   */
	registerServer(server: Server | IVirtualServer, port: number, hostname = "0.0.0.0"): void {
   ...
   */
	private async handleStreamingRequest(
		...
		if (!virtualServer) {
			...
			this.messageChannel?.port1.postMessage({ "type": "stream-end", "id": id });
        ...
        // onStart - called with headers
				(statusCode: number, statusMessage: string, respHeaders: Record<string, string>) => {
					...
					this.messageChannel?.port1.postMessage({
						"type": "stream-start",
						"id": id,
						"data": { "statusCode": statusCode, "statusMessage": statusMessage, "headers": respHeaders }
					});
				},
   ...
   */
	private notifyServiceWorker(type: string, data: unknown): void {
		if (this.serviceWorkerReady && this.messageChannel) {
			this.messageChannel.port1.postMessage({ "type": type, "data": data });
		}
	}
