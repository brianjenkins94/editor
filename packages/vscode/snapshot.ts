...
/** The DefinitelyTyped counterpart of a package: `react` → `@types/react`, `@scope/name` → `@types/scope__name`. */
...
function typeSurface(): SnapshotFile[] {
	...
	// Seed one package's decls; record an @types/* package's index.d.ts for a triple-slash ref (its globals must
	// be force-included since the in-browser tsserver doesn't auto-scan @types). Returns true if real types landed.
	const seedPackage = (pkg: string): boolean => {
		...
		if (pkg.startsWith("@types/") && existsSync(path.join(dir, "index.d.ts"))) {
			typeRefs.push(`./node_modules/${pkg}/index.d.ts`);
		}
	...
	files.push({
		...
		"contents":
			...
			+ "// for @types/*, so their global types are force-referenced by path, and packages that ship no types\n"
			+ "// (or are deliberately shimmed) get ambient `any` declarations. A project file, NOT an @types package.\n"
			+ (refs === "" ? "" : refs + "\n") + body + "\n"
	});

	// Seed an EMPTY ata-ambient.d.ts so it EXISTS in the project from boot. ATA (ata.ts writeAmbient) rewrites this
	// file at runtime with triple-slash references to the @types it acquires on demand. If it doesn't already
	// exist, that FIRST runtime write is a NEW root .d.ts appearing MID project-load, which makes the TS server
	// restart loading without a matching `projectLoadingFinish` — so the "Analyzing '…' and its dependencies"
	// status never clears on a FIRST load (it does on a refresh, where the file is restored from IndexedDB before
	// the load). Seeding it makes ATA's write a benign UPDATE to an existing project file instead of an ADD.
	files.push({
		"path": `${FOLDER}/ata-ambient.d.ts`,
		"contents": "// Auto-generated by ata.ts at runtime — force-references @types acquired on demand.\n// Seeded empty so its first runtime write is an UPDATE, not a project-reloading ADD.\n"
	});
