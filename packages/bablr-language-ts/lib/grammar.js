...
const optionalBodyHook = function* (instr) {
...
const asyncAndTypeParametersHook = function* () {
...
const noSemiStatements = [
  'Switch', 'Block', 'DoWhile', 'EmptyStatement', 'TryCatch',
...
export class TypeScriptAtrivial extends ESNext.atrivial {
  ...
  *Keyword({ literalValue, s }) {
    ...
    if (s().span.name !== 'Escape' && /^[a-zA-Z]/.test(word)) {
      if (!(yield match(m`/[ \n\r\t]|\/\/|\/\*|$|[*&^%$#@!?(){}[\].,/\\'";:=+|\u0061~-]|<|>/`))) {
        yield fail();
      }
    }
  ...
  *If() {
    ...
    let endsWithBlock = !!(yield match(m`'{'`));
  ...
  *EscapeSequence({ s }) {
    ...
    if (yield match(m`/[xu]/`)) {
      ...
      let value = parseInt(printSource(get('value', code.node)), 16);
  ...
  *LogicExpression({ s, props: { power, noIn = false } }) {
    ...
    if (binaryPowers[op] !== undefined) {
      ...
      return void (yield eat(m`<BinaryExpression />`, o({ op, power: power_, noIn })));
    }
    if (assignmentOperators.has(op) && power_ >= 15) {
      if (!assignableTypes.has(getRoot(shifted).value.name.description)) return;
      return void (yield eat(m`<AssignmentExpression />`, o({ op })));
    }
  }
  ...
  *BinaryExpression({ props: { op, noIn } }) {
    ...
    yield eat(m`right+$: <_Expression />`, o({ power: op === '**' ? ownPower : ownPower - 1, noIn }));
  }
  ...
  *Type({ s, props: { power = typePowers.conditional } }) {
    ...
    if (!s().shifted) {
      ...
      res = yield eat(m`<_TypeTail />`, o({ power }));
    }
    if (res && !(yield match(m`/$/`))) {
      return r(shiftMatch(m`<_Type />`, o({ power })));
    }
  }
...
export const enhance = (Grammar) => triviaEnhancer(
  {
