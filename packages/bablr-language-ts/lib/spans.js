...
import { OpenNodeTag, CloseNodeTag, LiteralTag, ReferenceTag, ShiftTag, GapTag } from '@bablr/agast-helpers/symbols';
import TypeScript from './grammar.js';

const COVER = Symbol.for('_');

const matcherFor = (production) =>
  production === 'Expression' ? m`<Expression />` : production === 'Statement' ? m`<Statement />` : m`<Program />`;
...
export function cstSpans(src, production = 'Program') {
  ...
  for (const tag of streamParse(TypeScript, matcherFor(production), src)) {
    ...
    if (kind === ReferenceTag) {
      ...
      spans.push(span);
      if (entry.trivia) triviaDepth--;
      else lastClosed = span;
    }
  }
