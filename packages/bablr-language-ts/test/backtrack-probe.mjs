...
import { m, eat, eatMatch, startSpan, endSpan, match } from '@bablr/helpers/grammar';
import { printSource } from '@bablr/agast-helpers/tree';
import { freezeClass } from '@bablr/agast-helpers/object';
import { TypeScriptAtrivial, enhance } from '../lib/grammar.js';
const run = (label, Cls, input) => {
  freezeClass(Cls);
  try { const out = printSource(treeParse(enhance(Cls), m`<Program />`, input)); console.log(label.padEnd(62), JSON.stringify(input).padEnd(6), out === input ? 'OK' : 'DIFF ' + JSON.stringify(out)); }
  catch (e) { console.log(label.padEnd(62), JSON.stringify(input).padEnd(6), 'FAIL', e.message.split('\n')[0]); }
};
const Word = { *Word() { yield eat(m`/[a-z]+/`); } };
...
run('same, with a >-guarded span opened inside A before failing', class extends TypeScriptAtrivial {
  *Program() { yield eatMatch(m`a$: <A '<' />`); const g = yield match(m`/$/`); if (g) throw new Error('leaked > guard'); yield eat(m`b$: <B />`); }
