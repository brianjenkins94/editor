...
function run(label, program, word, input) {
	const Cls = class extends TypeScriptAtrivial {
		*Program() {
			yield* program();
		}
	...
	try {
		...
		console.log(label.padEnd(60), JSON.stringify(input).padEnd(10), "FAIL", error.message.split("\n")[0]);
	}
}

const wordMatcher = m`a: <*Word />`;
const punctuatorMatcher = m`close: <*Punctuator />`;
const restMatcher = m`rest: <*Rest />`;

run("W1 string token a]b under ]-guard, then ]", function *() {
	yield startSpan("X", "]");
	yield eat(wordMatcher);
	yield endSpan();
	yield eat(punctuatorMatcher);
}, m`'a]b'`, "a]b]");
run("W1 regex token /a\\]b/ under ]-guard, then ]", function *() {
	yield startSpan("X", "]");
	yield eat(wordMatcher);
	yield endSpan();
	yield eat(punctuatorMatcher);
}, mRaw("/a\\]b/"), "a]b]");
run("W1 control: same, unguarded", function *() {
	yield startSpan("X");
	yield eat(wordMatcher);
	yield endSpan();
	yield eat(punctuatorMatcher);
}, m`'a]b'`, "a]b]");
run("W1 control: token without guard char", function *() {
	yield startSpan("X", "]");
	yield eat(wordMatcher);
	yield endSpan();
	yield eat(punctuatorMatcher);
...
run("W2 guard /\\bof\\b/, token /\\w+/ on typeof", function *() {
	yield startSpan("X", mRaw("/\\bof\\b/"));
	yield eat(wordMatcher);
	yield endSpan();
}, mRaw("/\\w+/"), "typeof");
run("W2 control: same on \"type\"", function *() {
	yield startSpan("X", mRaw("/\\bof\\b/"));
	yield eat(wordMatcher);
	yield endSpan();
}, mRaw("/\\w+/"), "type");
...
run("W3 lookahead match(/a,b/) under ,-guard", function *() {
	yield startSpan("X", ",");
	const la = yield match(m`/a,b/`);

	yield endSpan();
	if (!la) {
		throw new Error("lookahead saw nothing");
	}

	yield eat(restMatcher);
}, null, "a,b");
