...
import { m, eat, eatMatch, match, startSpan, endSpan } from '@bablr/helpers/grammar';
import { printSource } from '@bablr/agast-helpers/tree';
import { freezeClass } from '@bablr/agast-helpers/object';
import { TypeScriptAtrivial, enhance, mRaw } from '../lib/grammar.js';

const run = (label, program, word, input) => {
  ...
  freezeClass(Cls);
  try { const out = printSource(treeParse(enhance(Cls), m`<Program />`, input)); console.log(label.padEnd(60), JSON.stringify(input).padEnd(10), out === input ? 'OK' : 'DIFF ' + JSON.stringify(out)); }
  catch (e) { console.log(label.padEnd(60), JSON.stringify(input).padEnd(10), 'FAIL', e.message.split('\n')[0]); }
};
const W = m`a: <*Word />`, P = m`close: <*Punctuator />`, R = m`rest: <*Rest />`;
...
run('W2 control: same on "typeof" with JS RegExp semantics', function* () { const ok = /\w+/y.exec('typeof')[0] === 'typeof' && !/\bof\b/y.test('typeof'.slice(4)) ; yield eat(W); }, mRaw('/\\w+/'), 'typeof');

run('W3 lookahead match(/a,b/) under ,-guard', function* () { yield startSpan('X', ','); const la = yield match(m`/a,b/`); yield endSpan(); if (!la) throw new Error('lookahead saw nothing'); yield eat(R); }, null, 'a,b');
run('W3 control: same lookahead, unguarded', function* () { yield startSpan('X'); const la = yield match(m`/a,b/`); yield endSpan(); if (!la) throw new Error('lookahead saw nothing'); yield eat(R); }, null, 'a,b');
