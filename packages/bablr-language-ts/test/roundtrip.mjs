...
const cases = [];
let skipped = 0;
for await (const c of loadCorpus({ corpora, size })) {
  if (c.skip !== undefined) {
    skipped++;
    continue;
  }
...
const runOn = (worker, served) =>
  new Promise((resolveSlot) => {
    const loop = () => {
      ...
      worker.postMessage({ seq, source: c.source });
    };
    loop();
  });
...
const meta = { size, corpora, fast, grammar: process.env.GRAMMAR ?? 'lib/grammar.js', ran: new Date().toISOString() };
writeFileSync(out, JSON.stringify({ ...meta, results: stripped }, null, 1)); // raw results first: analysis may fail
const analysis = analyze(cases, results, { size, started });
console.log('\n' + analysis.text);
