...
export const parse = (name, source) =>
  ts.createSourceFile(name || 'case.ts', source, ts.ScriptTarget.Latest, true, /\.js$/.test(name || '') ? ts.ScriptKind.JS : ts.ScriptKind.TS);
  ...
  const visit = (node) => {
    switch (node.kind) {
      ...
      case K.FunctionExpression: {
        const a = hasMod(node, K.AsyncKeyword), g = !!node.asteriskToken;
      ...
      case K.ArrayBindingPattern: {
        let ctx = 'nested';
        const p = node.parent;
        if (p?.kind === K.Parameter) ctx = 'param';
        else if (p?.kind === K.VariableDeclaration) {
      ...
      case K.BinaryExpression: {
        ...
        if (op === K.CommaToken) add('Comma');
        else if ([K.QuestionQuestionToken, K.AsteriskAsteriskToken, K.InKeyword, K.InstanceOfKeyword, K.QuestionQuestionEqualsToken, K.BarBarEqualsToken, K.AmpersandAmpersandEqualsToken, K.AsteriskAsteriskEqualsToken, K.GreaterThanGreaterThanGreaterThanEqualsToken, K.GreaterThanGreaterThanGreaterThanToken].includes(op)) add(`Binary:${text}`);
        else if (op !== K.EqualsToken && text.endsWith('=') && !/^[=!<>]=+$/.test(text)) add('CompoundAssignment');
      ...
      case K.NumericLiteral: {
        ...
        if (/^0[xX]/.test(t)) add('Numeric:hex');
        else if (/^0[oO]/.test(t)) add('Numeric:octal');
        else if (/^0[bB]/.test(t)) add('Numeric:binary');
        else if (/^0\d/.test(t)) add('Numeric:legacyOctal');
        else {
          if (/^\./.test(t)) add('Numeric:leadingDot');
          if (/\.$/.test(t) || /\.[eE]/.test(t)) add('Numeric:trailingDot');
          if (/[eE]/.test(t)) add('Numeric:exponent');
    ...
    if (ts.canHaveModifiers(node)) for (const m of ts.getModifiers(node) ?? []) if (TS_ONLY_MODIFIERS.has(m.kind) || (m.kind === K.ConstKeyword && node.kind !== K.VariableStatement)) add(`TS:mod:${ts.tokenToString(m.kind)}`);
...
export function textFeatures(source) {
  ...
  if (source.includes('\r\n')) out.add('CRLF');
  else if (source.includes('\r')) out.add('CR');
...
export function statementAt(sf, pos) {
  for (const s of sf.statements) if (pos < s.end) return s;
  return null;
}
