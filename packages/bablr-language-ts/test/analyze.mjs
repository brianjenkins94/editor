...
import { parse, usesTsSyntax, locate, bucketKey, features, textFeatures, statementAt } from './classify.mjs';
...
export function analyze(cases, results, { size, started, top = 40 } = {}) {
  ...
  for (const r of rows) if (r.status === 'pass') for (const f of r.fileFeatures) passCount.set(f, (passCount.get(f) ?? 0) + 1);
  const suspectTable = (group, title) => {
    ...
    const list = [...failCount]
      .map(([f, fails]) => ({ f, fails, passes: passCount.get(f) ?? 0 }))
  ...
  for (const r of rows) {
    ...
    tsCases++;
    for (const f of r.fileFeatures) if (f.startsWith('TS:')) tsCount.set(f, (tsCount.get(f) ?? 0) + 1);
  }
  out();
  ...
  return { rows, tally: Object.fromEntries(tally), suspects, tsSuspects, buckets: Object.fromEntries(sorted), tsCensus: Object.fromEntries(tsCount), text: lines.join('\n') };
}
