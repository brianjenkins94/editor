 ...
 */
import type { TypedHostCallSite } from "../../src/typed.ts";
import assert from "node:assert/strict";
import { test } from "node:test";
import ts from "typescript";
import { createVM, interpret, interpretAsync } from "../../src/index.ts";
...
test("hostGuard.sanitize sees every property read that crosses from the host, including intrinsic reflection", () => {
	...
	const result = interpret(`[].constructor.constructor`, { "hostGuard": { "sanitize": sanitize } });
...
test("hostGuard.sanitize applies to values a host function returns and to awaited values", async () => {
	...
	assert.equal(interpret(`host()`, { "globals": { "host": () => 4 }, "hostGuard": { "sanitize": sanitize } }), 40);
	assert.equal(await interpretAsync(`(async () => await Promise.resolve(5))()`, { "hostGuard": { "sanitize": sanitize } }), 50);
});
...
test("resolveModule is the import seam: a host decides what `import` and `import()` resolve to", async () => {
	...
	assert.equal(await interpretAsync(`import("y").then((m) => m.named)`, { "resolveModule": resolveModule }), 7);
});
...
test("typed layer: a guard can answer with a stand-in shaped like the call's declared result type", () => {
	...
	const { vm } = createTypedVM(code, { "globals": { "load": load }, "hostGuard": { "beforeCall": beforeCall } });
...
test("typed layer: signature() and argumentType() expose the declared parameter and argument types", () => {
	...
	const { vm } = createTypedVM(`declare function send(url: URL, body: string): Promise<number>; send(new URL("https://x"), "hi")`, { "globals": { "send": () => 1, "URL": URL }, "hostGuard": { "beforeCall": beforeCall } });
