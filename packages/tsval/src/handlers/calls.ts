...
export function templateObject(vm: Machine, node: ts.TaggedTemplateExpression): readonly string[] {
	...
	if (strings === undefined) {
		...
		const add = (lit: ts.TemplateLiteralLikeNode): void => {
			cooked.push(cookedTemplateText(lit)!); // (undefined for an illegal escape — allowed in a TAGGED template)
	...
	if (frame.phase < AFTER_REF) {
		...
		if (isGuestFunction(calleeVal)) {
			...
			if (meta.isGenerator && meta.isAsync) {
				...
				vm.push(vm.createAsyncGenerator(meta, frame.thisArg, args));
			...
			if (meta.isGenerator) {
				...
				vm.push(vm.createGenerator(meta, frame.thisArg, args));
