...
import { FULL_ROOT, SAMPLE_ROOT, TS_CASES_PIN, hasCorpus, loadTsCases, policySkip } from "../test/differential/ts-cases-corpus.ts";
...
for (const name of ["LICENSE.txt", "ThirdPartyNoticeText.txt"]) if (fs.existsSync(path.join(FULL_ROOT, name))) fs.copyFileSync(path.join(FULL_ROOT, name), path.join(SAMPLE_ROOT, name));
