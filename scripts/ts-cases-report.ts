...
const bucket = <T>(items: T[], key: (t: T) => string): [string, number][] => {
...
await runner.close();
const lateRejections = runner.lateRejections;
...
if (jsonOut !== undefined) fs.writeFileSync(jsonOut, JSON.stringify({ counts, mismatches }, null, 2));
